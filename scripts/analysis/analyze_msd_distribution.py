...
def plot_histograms(df, plot_prefix, logy=False):
    ...
    for index, step in enumerate(steps):
        ...
        col = index % n_cols
        ax = axes[row][col]
        values = df.loc[df["step"] == step, "r2"].astype(float).values
...
def plot_boxplot(df, plot_prefix):
    steps = sorted(df["step"].unique())
    data = [df.loc[df["step"] == step, "r2"].astype(float).values for step in steps]
    ...
    out_path = f"{plot_prefix}_r2_boxplot_by_step.png"
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    return out_path


def plot_moment_curves(summary_df, plot_prefix):
    ...
    ax.set_xlabel("step")
    ax.set_ylabel(r"$r^2$")
    ax.set_title(r"$r^2$ summary statistics by step")
    ax.grid(True, which="both")
    ax.legend()
    fig.tight_layout()

    out_path = f"{plot_prefix}_r2_moments_by_step.png"
    fig.savefig(out_path, dpi=300)
    plt.close(fig)
    return out_path
...
def main():
    ...
    hist_png = plot_histograms(df, args.plot_prefix, logy=False)
    hist_log_png = plot_histograms(df, args.plot_prefix, logy=True)
    boxplot_png = plot_boxplot(df, args.plot_prefix)
    moments_png = plot_moment_curves(summary_df, args.plot_prefix)
